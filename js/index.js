...
$(document).ready(function () {
    ...
    $('.btn-wechat').click(function () {
        ...
        if ($('#head-qr').css('display') !== 'none') {
            setTimeout(function () {
                $(window).one('click', function () { $('#head-qr').hide(); });
            }, 350);
        }
    });
    ...
    if (lang === 'en') {
        ...
        $('.show-cn').hide();
        $('.show-en').show();
        localStorage && ( localStorage.lang = 'en' );
        ...
        if (getUrlParam('lang') == 'cn') {
            localStorage && (localStorage.lang = 'cn');
        }
    }
    try{
        loadTop100(lang);
    ...
    // 数据页面
    if($('#datasecondnav').length>0){
        if(window.location.href.indexOf('#data-company')>0){
            $('.tab-content').hide();
            $('#data-company').show();
            $(window).scrollTo($('#data-company'));
        }
        if(window.location.href.indexOf('#data-city')>0){
            $(window).scrollTo($('#data-city'));
        }
...
var items = ['公共安全', '智慧交通', '健康生活', '未来商圈', '环境保护', '社会治理', '工业生产', '信息消费', '企业信用'];
...
function loadTop100(lang){
    if(show_top_100){
        $('.btn-finalist').show();
        var current = '';
        var list = $('#top-100-list');
        if(lang==='en'){
            finalists.sort(comp);
        }
        ...
        for(var i = 0; i < finalists.length; i++) {
            var f = finalists[i];
            var p = f.team_pinyin.toUpperCase()[0];
            if(lang==='en'){
                p = f.team_en.toUpperCase()[0];
            }
            if (p!==current){
                current = p;
                list.append($('<li></li>').addClass('head').text(p));
            }
            var t = $($('#top-100-template').html());
            if(lang!=='en'){
                t.find('.team').text(f.team);
                t.find('.project').text(f.project);
            }else{
                t.find('.team').text(f.team_en);
                t.find('.project').text(f.project_en);
            }
            list.append(t);
        }
    }
